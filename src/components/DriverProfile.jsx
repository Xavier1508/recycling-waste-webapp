        ...
        return (
            <Link href={`/track/${activeTask.pickup_id}`} legacyBehavior>
                <a className="block hover:scale-105 transition-transform group">
                    <div className="nav-button-container bg-green-100 border border-green-300">
                        <FaMapMarkedAlt className="nav-button-icon text-green-600" />
                        <span className="nav-button-text text-green-700">Lacak Tugas</span>
                    </div>
                </a>
            </Link>
        );
            ...
            <Link 
                href={{
                    ...
                    query: { 
                        ...
                        avatar: driverData.profile_picture_url
                    }
                }} 
                legacyBehavior
            >
                <a className="block hover:scale-105 transition-transform group">
                    <div className="nav-button-container">
                        <FaMapMarkedAlt className="nav-button-icon" />
                        <span className="nav-button-text">Lihat Peta</span>
                    </div>
                </a>
            </Link>
        );
...
const DriverProfile = () => {
    ...
    const fetchInitialData = useCallback(async () => {
        ...
        try {
            ...
            if (!profileData || !profileData.driver_id) {
                throw new Error("Profil driver tidak valid atau tidak ditemukan.");
            }
            ...
            if (!socket.connected) {
                ...
                socket.on('new_pickup_offer', (offerDetails) => {
                    setActiveTask(currentTask => {
                        if (!currentTask) setCurrentOffer(offerDetails);
                        return currentTask;
                    });
                });
            }
                ...
                <div className="lg:col-span-1 bg-white rounded-xl p-6 shadow-lg">
                    ...
                    <div className="space-y-4">
                        <div className="p-4 bg-blue-50 rounded-lg">
                            ...
                            <p className="text-3xl font-bold text-blue-900">{driverStats.tasks_today}</p>
                        </div>
                        <div className="p-4 bg-green-50 rounded-lg">
                            <p className="text-sm text-green-700 font-semibold">Tugas Selesai (Total)</p>
                            <p className="text-3xl font-bold text-green-900">{driverStats.tasks_completed}</p>
                        </div>
                        <div className="p-4 bg-yellow-50 rounded-lg">
                            <p className="text-sm text-yellow-700 font-semibold">Menunggu Konfirmasi</p>
                            <p className="text-3xl font-bold text-yellow-900">{driverStats.tasks_pending_confirmation}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="p-4 grid grid-cols-3 gap-3 text-center bg-white rounded-xl shadow-lg">
                        <Link href="/historydriver" className="block hover:scale-105 transition-transform group">
                            <div className="nav-button-container">
                                <FaHistory className="nav-button-icon" />
                                <span className="nav-button-text">Riwayat</span>
                            </div>
                        </Link>
                        <button className="block hover:scale-105 transition-transform group">
                            <div className="nav-button-container">
                                <FaBell className="nav-button-icon" />
                                <span className="nav-button-text">Notifikasi</span>
                            </div>
                        </button>
                        <ViewMapButton activeTask={activeTask} driverData={driverData} />
                    </div>
                    <div className="p-4 grid grid-cols-3 gap-3 text-center bg-white rounded-xl shadow-lg">
                        <button className="block hover:scale-105 transition-transform group">
                            <div className="nav-button-container">
                                <FaWallet className="nav-button-icon" />
                                <span className="nav-button-text">Dompet</span>
                            </div>
                        </button>
                        <a href="https://wa.me/628113926360" target="_blank" rel="noopener noreferrer" className="block hover:scale-105 transition-transform group">
                            <div className="nav-button-container">
                                <FaHeadset className="nav-button-icon" />
                                <span className="nav-button-text">Hubungi CS</span>
                            </div>
                        </a>
                        <button className="block hover:scale-105 transition-transform group">
                            <div className="nav-button-container">
                                <FaQuestionCircle className="nav-button-icon" />
                                <span className="nav-button-text">Bantuan</span>
                            </div>
                        </button>
                    </div>
                    <DashboardMap driverData={driverData} activeTask={activeTask} />
                </div>
